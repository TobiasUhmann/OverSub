console.log('load SubBox HTML');
var htmlURL = chrome.extension.getURL('content/SubBox/SubBox.html');
$.ajax({ url: htmlURL, dataType: 'html', success: function (htmlString) {
	var x = $(htmlString).appendTo(document.body);
	console.log(x);
	
	console.log('add JS functionality to SubBox');
	x.draggable();
	x.resizable();
}});
